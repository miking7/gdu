...
- **Scope filters are rejected for Parquet, honored for JSON.** `-o x.parquet` (or
  `--output-format parquet`) combined with `--top`, `--depth` or `--summarize` is an error at
  startup. A snapshot's footer manifest claims a *complete* scan — its rows and totals feed the
  timeline, baselines, launcher recency and go-live — so a scope-filtered file would silently lie
  about being a full scan, and every downstream comparison would be against a partial tree.
  `--export-threshold` stays the one sanctioned lossy knob: it is totals-preserving and its value is
  recorded in the manifest. Plain JSON has no such identity to protect, so it adopts upstream's
  filters verbatim, applying the filter first and then the rollup to the already-filtered tree.
...
**Quitting is snapshot-aware.** Because the default archives every deliberate scan, upstream's
"your results are unsaved — press E to export" confirmation would fire on exactly the scans that
*were* saved. So the confirmation is gated on whether anything is actually at risk. Post-scan, it
fires only when the on-screen results were never recorded — snapshots disabled, a transient scan,
or a failed write, tracked by `unsavedScanDuration` (bumped at completion only when the scan did not
produce a snapshot; a recorded scan leaves it untouched) — and then only past a ~3s threshold.
Mid-scan, it fires for a recording scan (any duration, since abandoning it loses a snapshot) or for
any scan already running past the threshold. This folds upstream's #593 long-scan confirm into the
fork's recording model under a single `no-confirm-quit` switch, rather than running two parallel
quit paths — and every exit still routes through the one drain-aware chain that waits on background
compaction.
...
- **Mid-scan preview is the live position, momentarily rendered** (upstream #594, folded in). `Tab`
  swaps the progress screen for a read-only view of the partial tree found so far. It is deliberately
  *page-level* state (`previewing`/`previewSavedDir`/`progressFlex`), **not** a `tui.view`: a View is
  a point on the timeline, and the preview *is* the live point — showing the present, not the past —
  so making it a View would double-count the live position and fight the fold rule. The two gestures
  compose cleanly because they move in opposite directions: `Tab` browses the *present* being built,
  `[`/`]` browse the *past* already recorded. From a preview, `[`/`]` leave it and step the timeline
  (a preview is point-in-time and never auto-re-previews); a scan finishing while previewing renders
  the final tree, since a previewing user is watching the live position. `q` in a preview routes
  through the normal quit chain (a deliberate divergence from upstream, which leaves `q` dead there)
  — the chain already confirms an in-flight recording scan before abandoning it.
