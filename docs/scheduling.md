...
`gdu --save-snapshots always` on a schedule. **This guide assumes you want a whole-disk scan as `root`**, which is
...
**`--owner youruser`** and it writes the snapshot into that user's `~/.local/share/gdu/snapshots`
(their `XDG_DATA_HOME` environment can't be seen from a root daemon, so the `~/.local/share` default
is always used) and hands every file it creates back to them. That one flag is all the units below
need — replace `youruser` with your login name.
...
ExecStart=/usr/local/bin/gdu -np --owner youruser --save-snapshots always /
...
After it runs, check `ls -l ~/.local/share/gdu/snapshots/` — the `snapshot_*.parquet` file should be
...
30 2 * * *  root  /usr/local/bin/gdu -np --owner youruser --save-snapshots always / >> /var/log/gdu-scan.log 2>&1
```
...
ls -l ~/.local/share/gdu/snapshots/  # the new snapshot should be owned by you, not root
```
