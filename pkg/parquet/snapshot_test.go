...
func TestSnapshotFileName(t *testing.T) {
	...
	assert.Equal(t, "snapshot_20260619T053015_users_michael.parquet",
...
func TestSaveSnapshotRoundTrip(t *testing.T) {
	...
	path, _, createdDir, err := SaveSnapshot(root, dir, 0, now)
	require.NoError(t, err)
	// t.TempDir() already exists, so this save did not create the archive dir.
	assert.False(t, createdDir)
	// Root is /tmp/root, so the filename carries the "tmp_root" slug.
	assert.Equal(t, filepath.Join(dir, "snapshot_20260619T053000_tmp_root.parquet"), path)
...
func TestSaveSnapshotCollision(t *testing.T) {
	...
	first, _, _, err := SaveSnapshot(root, dir, 0, now)
	require.NoError(t, err)
	second, _, _, err := SaveSnapshot(root, dir, 0, now) // same second must not overwrite
	require.NoError(t, err)

	assert.NotEqual(t, first, second)
	assert.Equal(t, filepath.Join(dir, "snapshot_20260619T053000_tmp_root.parquet"), first)
	assert.Equal(t, filepath.Join(dir, "snapshot_20260619T053000_tmp_root_1.parquet"), second)
...
func TestSaveSnapshotCreatesDir(t *testing.T) {
	...
	_, _, createdDir, err := SaveSnapshot(root, dir, 10*mib, time.Unix(1700000000, 0).UTC())
	require.NoError(t, err)
	assert.True(t, createdDir, "creating the archive dir must be reported for the announce")
