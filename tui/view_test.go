...
func TestQuitMidScanConfirms(t *testing.T) {
	...
	// A short transient scan was never going to save and has not run long
	// enough to warn about, so quit proceeds directly. (A transient scan past
	// the 3s threshold would still confirm — that path is covered elsewhere.)
	out := &bytes.Buffer{}
	ui.output = out
