		...
		func(itemCount int64, totalUsage int64, currentItem string) {
			...
			ui.app.QueueUpdateDraw(func() {
				progressView.SetText("Total items: " +
					...
					// Tab peeks at the partial tree found so far (present tense).
					"[white:black:-]\n\nPress Tab to preview results found so far" +
					// The scan-wait time-travel nudge (browse the past), set
					// asynchronously once covering history is found; empty otherwise.
					ui.scanNudge)
			})
