...
func TestHostIsForeign(t *testing.T) {
	assert.False(t, HostIsForeign("", "local"), "no host stamped = this machine")
	assert.False(t, HostIsForeign("local", "local"), "exact match = this machine")
	assert.False(t, HostIsForeign("", ""), "both empty")
	assert.True(t, HostIsForeign("other", "local"), "a different host is foreign")
	assert.True(t, HostIsForeign("Mac-3.local", "Mac-2.local"), "renamed-machine drift reads as foreign")
}

func TestHostnameBestEffort(t *testing.T) {
	// Mirrors os.Hostname(): on any real machine it is non-empty and matches
	// what CollectScanIdentity stamps, so a reader recognizes "this machine".
	assert.Equal(t, CollectScanIdentity().Host, HostnameBestEffort())
}
